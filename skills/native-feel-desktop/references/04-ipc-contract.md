...
This is a proven pattern: a shipping app reverse-engineered in `07-evidence-raycast.md` exposes exactly this shape — a `Coordinator` interface plus callback interfaces for events and logs, all generated by UniFFI. Use UniFFI; the symbol-level forensics live in `07`.
...
**Concrete hot-loop to avoid (T6):** a React effect posts a message on every keystroke (`onChange → sendToHost`); Node re-queries and broadcasts a result event; the event re-renders the tree, which re-fires the effect. Each hop looks cheap, but at ~10 keystrokes/sec across four processes it's a storm — and the profiler shows nothing damning per call, only aggregate jank. Fix: debounce the send (~50 ms), make the query cancelable, and key the effect so a broadcast it caused doesn't re-trigger it. The discipline is T6 — every crossing is intentional, batched, and observable.
